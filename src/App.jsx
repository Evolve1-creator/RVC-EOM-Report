...
const styles = {
  ...
  hiddenInput: { position: "absolute", width: 1, height: 1, padding: 0, margin: -1, overflow: "hidden", clip: "rect(0,0,0,0)", whiteSpace: "nowrap", border: 0 },
...
function normalizePatientName(value) {
  return normalizeWhitespace(value).replace(/\s+,/g, ",").replace(/,\s+/g, ", ").replace(/\s+/g, " ").trim();
}

function parseAmount(line) {
  const matches = [...line.matchAll(/-?\d{1,3}(?:,\d{3})*\.\d{2}/g)].map((m) => Number(m[0].replace(/,/g, "")));
...
function isInsuranceLine(line) {
  return /\b(era|eft|payer|insurance|medicare|regence|aetna|allstate|farmers|state farm|usaa|samba|mutual of omaha|moda|aarp|aflac|bcbs|direct deposit|check)\b/i.test(line);
}

function isPatientLine(line) {
  return /\b(patient|cash|visa|mastercard|discover|amex|credit card|debit card|copay|co-pay)\b/i.test(line);
}
...
function splitPages(text) {
  return text.split(/(?=Page\s+\d+)/i).map((x) => x.trim()).filter(Boolean);
}
...
function parsePatientPayments(rawText) {
  ...
  for (const page of splitPages(rawText)) {
    ...
    for (const line of lines) {
      ...
      if (isInsuranceLine(line) && !isPatientLine(line)) { row.insurancePayments += amount; row.insuranceLines.push(detail); }
      else if (isPatientLine(line) && !isInsuranceLine(line)) { row.patientPayments += amount; row.patientLines.push(detail); }
    }
  }
...
export default function App() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return q ? rows.filter((row) => row.patient.toLowerCase().includes(q)) : rows;
  }, [rows, search]);
  ...
  async function handleUpload(event) {
    ...
    try {
      ...
      console.error(err);
      setError("The PDF could not be parsed. This build expects the same EOD payment-report format used in this chat.");
      setRows([]);
      setNotes([]);
      setReportPayerTotal(null);
    } finally {
      setLoading(false);
      event.target.value = "";
    }
    ...
    <div style={styles.page}>
      <div style={styles.wrap}>
        <div style={styles.hero}>
          ...
          <button type="button" style={styles.uploadButton} onClick={openPicker}>{loading ? "Processing..." : "Upload PDF"}</button>
          <input ref={fileInputRef} type="file" accept="application/pdf" style={styles.hiddenInput} onChange={handleUpload} />
        </div>
        ...
        {!!rows.length && <>
          <div style={styles.cardGrid}>
            <MetricCard title="Patients Found" value={String(totals.count)} />
            <MetricCard title="Insurance Payments Parsed" value={money.format(totals.insurance)} />
            <MetricCard title="Patient Payments Parsed" value={money.format(totals.patient)} />
            <MetricCard title="Report Payer Total" value={reportPayerTotal == null ? "—" : money.format(reportPayerTotal)} />
          </div>
          ...
          <div style={styles.tableWrap}>
            <div style={{ padding: 18, ...styles.toolbar }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                ...
                <button type="button" style={styles.button} onClick={() => exportRows(filteredRows, fileName)}>Export XLSX</button>
              </div>
            </div>
              ...
              <table style={styles.table}>
                <thead>
                  <tr>
                    ...
                    <th style={{ ...styles.th, ...styles.right }}>Insurance Payments Made in Month</th>
                    <th style={{ ...styles.th, ...styles.right }}>Patient Payments Made in Month</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredRows.map((row) => {
                    const open = !!expanded[row.patient];
                    return <React.Fragment key={row.patient}>
                      <tr>
                        <td style={styles.td}><button type="button" style={{ ...styles.button, padding: "6px 10px", borderRadius: 10 }} onClick={() => toggle(row.patient)}>{open ? "−" : "+"}</button></td>
                        <td style={{ ...styles.td, fontWeight: 700 }}>{row.patient}</td>
                        <td style={{ ...styles.td, ...styles.right, fontWeight: 700 }}>{money.format(row.insurancePayments)}</td>
                        <td style={{ ...styles.td, ...styles.right, fontWeight: 700 }}>{money.format(row.patientPayments)}</td>
                      </tr>
                      {open ? <tr><td colSpan={4} style={styles.td}><div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }}>
                        <div style={styles.detailBox}><div style={{ fontWeight: 800, marginBottom: 8 }}>Insurance lines</div>{row.insuranceLines.length ? row.insuranceLines.map((item, idx) => <div key={idx} style={{ marginBottom: 8 }}><div><strong>{item.date || "No date"}</strong> — {money.format(item.amount)}</div><div>{item.sourceLine}</div></div>) : <div>No lines found.</div>}</div>
                        <div style={styles.detailBox}><div style={{ fontWeight: 800, marginBottom: 8 }}>Patient lines</div>{row.patientLines.length ? row.patientLines.map((item, idx) => <div key={idx} style={{ marginBottom: 8 }}><div><strong>{item.date || "No date"}</strong> — {money.format(item.amount)}</div><div>{item.sourceLine}</div></div>) : <div>No lines found.</div>}</div>
                      </div></td></tr> : null}
                    </React.Fragment>;
                  })}
                </tbody>
