...
function parsePatientPayments(rawText) {
  ...
  for (const page of splitPages(rawText)) {
    ...
    for (const line of lines) {
      if (/^Payments for\b/i.test(line)) {
        currentSection = "payments";
        currentPatient = "";
        continue;
      }
      if (/^Charges for\b/i.test(line)) {
        currentSection = "charges";
        currentPatient = "";
        continue;
      }
      if (currentSection !== "payments") continue;
      if (/\b(Payer Totals|Patient Totals|Office Totals|Transaction Totals)\b/i.test(line)) {
        currentPatient = "";
        continue;
      }
      if (looksLikePatientName(line)) {
        currentPatient = normalizePatientName(line);
        if (!patients.has(currentPatient)) {
          patients.set(currentPatient, {
      ...
      if (insurance && !patient) {
        ...
        row.insuranceLines.push(detail);
      } else if (patient && !insurance) {
        row.patientPayments += amount;
        row.patientLines.push(detail);
      }
    }
  }
...
export default function App() {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (!q) return rows;
    return rows.filter((row) => row.patient.toLowerCase().includes(q));
  }, [rows, search]);

  const totals = useMemo(() => ({
    insurance: round2(rows.reduce((sum, row) => sum + row.insurancePayments, 0)),
    patient: round2(rows.reduce((sum, row) => sum + row.patientPayments, 0)),
    count: rows.length,
  }), [rows]);
      ...
      <div style={styles.wrap}>
        <div style={styles.hero}>
          ...
          <div style={styles.heroText}>
            Upload the EOD payment PDF and the app totals insurance payments made in the month by patient, then totals patient payments made in that same month for those same patients.
          </div>
          ...
          <>
            ...
            <div style={styles.tableWrap}>
              <div style={{ padding: 18, ...styles.toolbar }}>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 20 }}>Results</div>
                  <div style={styles.small}>Patient | Insurance Payments Made in Month | Patient Payments Made in Month</div>
                </div>
                    ...
                    {filteredRows.map((row) => {
                      ...
                      return (
                        <React.Fragment key={row.patient}>
                          <tr>
                            <td style={styles.td}>
                              <button style={{ ...styles.button, padding: "6px 10px", borderRadius: 10 }} onClick={() => toggle(row.patient)}>{open ? "−" : "+"}</button>
                            </td>
                              ...
                              <td colSpan={4} style={styles.td}>
                                <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }}>
                                  <div style={styles.detailBox}>
                                    <div style={{ fontWeight: 800, marginBottom: 8 }}>Insurance lines</div>
                                    {row.insuranceLines.length ? row.insuranceLines.map((item, idx) => (
                                      <div key={idx} style={{ marginBottom: 8 }}>
                                        <div><strong>{item.date || "No date"}</strong> — {money.format(item.amount)}</div>
                                        <div>{item.sourceLine}</div>
                                      </div>
                                    )) : <div>No lines found.</div>}
                                  </div>
                                  <div style={styles.detailBox}>
                                    <div style={{ fontWeight: 800, marginBottom: 8 }}>Patient lines</div>
                                    {row.patientLines.length ? row.patientLines.map((item, idx) => (
                                      <div key={idx} style={{ marginBottom: 8 }}>
                                        <div><strong>{item.date || "No date"}</strong> — {money.format(item.amount)}</div>
                                        <div>{item.sourceLine}</div>
                                      </div>
